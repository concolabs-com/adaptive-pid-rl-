...
## 1. Current figure set (16, all committed)
...
| 5.0 Training curves | `chapter05/figures/fig5_training_curves.png` | `make_all_figures.py` | Ch5 §5.1 |
| 5.1 RQ2 settling bars | `chapter05/figures/fig5_rq2_settling.png` | `make_all_figures.py` | Ch5 §5.3 |
| 5.2 Actuator sweep | `chapter05/figures/fig5_actuator_sweep.png` | `make_all_figures.py` | Ch5 §5.3 |
| 5.3 Stack ablation | `chapter05/figures/fig5_stack_ablation.png` | `make_all_figures.py` | Ch5 §5.4 |
| 5.4 RQ4 probe R² | `chapter05/figures/fig5_probe_r2.png` | `make_all_figures.py` | Ch5 §5.5 |
| 5.5 Anti-windup | `chapter05/figures/fig5_antiwindup.png` | `make_all_figures.py` | Ch5 §5.6 |
| 5.6 Anti-windup traces | `chapter05/figures/fig5_antiwindup_traces_{none,backcalc}.png` | `stage_baseline_antiwindup_pid.py` (copied from `benchmark_results/`) | Ch5 §5.6 |
| 5.7 MRAC heatmap | `chapter05/figures/fig5_mrac.png` | `make_all_figures.py` | Ch5 §5.6 |
| 5.8 Mass sweep | `chapter05/figures/fig5_mass_sweep.png` | `make_all_figures.py` | Ch5 §5.7 |
| 5.9 Gain regimes | `chapter05/figures/fig5_gain_regimes.png` | `make_all_figures.py` | Ch5 §5.9 |
| 5.10 Pendulum survival | `chapter05/figures/fig5_pendulum_survival.png` | `make_all_figures.py` | Ch5 §5.10 |

Regenerate everything:
```
venv/Scripts/python.exe thesis_doc/scripts/make_diagrams.py          # Ch3 diagrams (3.1-3.4)
venv/Scripts/python.exe thesis_doc/scripts/make_all_figures.py       # Ch5 data figures from CSVs
venv/Scripts/python.exe thesis_doc/scripts/make_chapter_diagrams.py  # Ch1/2/3/4/6 conceptual diagrams
```

**Conceptual figures (generated by `make_chapter_diagrams.py`, pending embed).**
14 schematic/analytical figures — block diagrams and plots of the *equations*
(PID loop, windup, anti-windup, PPO clip, terminal-speed/identifiability, reward
shaping, pendulum cascade, scenario map, metric trace, capability-cost, decision
tree). Each lives under its own `chapter{01,02,03,04,06}/figures/figN_*.png`.
They are **now embedded** in their target sections (Ch1 §1.2/§1.3, Ch2
§2.1/§2.2/§2.4, Ch3 §3.2/§3.8, Ch4 §4.1/§4.2, Ch6 §6.4). Every chapter folder
(and `appendices/`) has a `FIGURES_NEEDED.md` holding the prioritized list, the
target section, copy-paste `![]()` snippets, and the remaining
author-by-hand (📐) and data (🔶) candidates not yet built.
...
pandoc \
  ...
  --pdf-engine=xelatex \
  --toc --number-sections \
  --resource-path=.:thesis_doc:thesis_doc/chapter01:thesis_doc/chapter02:thesis_doc/chapter03:thesis_doc/chapter04:thesis_doc/chapter05:thesis_doc/chapter06 \
  -V geometry:margin=1in -V documentclass=report -V fontsize=11pt
```

Notes:
- `--resource-path` lets Pandoc resolve the relative image paths
  (`figures/…` from both Ch3 and Ch5, each resolved against its own chapter
  dir). Adjust if you reorganize.
