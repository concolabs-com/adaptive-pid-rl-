...
def run_episode(env, scenario_name: str, mass: float, friction: float, seed: int) -> dict:
    ...
    # Metrics must use the env's actual target (V4 = 5 m, V5 no-reset = 8 m),
    # not the module-level constant — otherwise no-reset errors are measured
    # against the wrong setpoint.
    ...
    errors_arr = target - positions_arr
    settling_s, settled = compute_settling_time(errors_arr, dt)
    overshoot = float(max(np.max(positions_arr) - target, 0.0))
...
def plot_trajectories(scenario_results: dict, out_path: Path, title: str, scenarios: list) -> None:
    ...
    for i, (scenario_name, mass, friction) in enumerate(scenarios):
        ...
        ep_target = r.get("_target", TARGET_POS)
        ax0.axhline(ep_target, color="k", linestyle="--", linewidth=0.8, label="target")
        ax0.axhline(ep_target + TOLERANCE, color="gray", linestyle=":", linewidth=0.6)
        ax0.axhline(ep_target - TOLERANCE, color="gray", linestyle=":", linewidth=0.6)
