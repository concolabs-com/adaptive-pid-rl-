...
class EvalScenario:
    ...
    friction: float
    actuator: float = 1.0
    # Relative half-width bands for eval protocol v2: per-episode physics are
    # sampled as value * (1 + U(-band, +band)) so the 10 eval episodes per
    # scenario are genuinely different (honest variance) instead of 10
    # identical deterministic rollouts.
    mass_band: float = 0.0
    friction_band: float = 0.0
    actuator_band: float = 0.0
...
class Stage2Config:
    ...
    eval_only: bool
    actuator_strength_range: tuple[float, float]
    disturbance_actuator_scale_range: tuple[float, float]
    eval_protocol: str
...
# Eval protocol v2 scenario set: adds the actuator-strength axis (training
# range 0.6-1.4) and per-episode sampling bands. Target distance is also
# jittered per episode (see prepare_eval_episode). OOD rows probe beyond the
# training boundaries on each axis.
SCENARIOS_V2 = [
    EvalScenario(
        "Standard", mass=10.0, friction=1.0, actuator=1.0, mass_band=0.10, friction_band=0.10, actuator_band=0.05
    ),
    EvalScenario(
        "Heavy Weak Motor",
        mass=18.0,
    ...
    EvalScenario(
        "Light and Grippy", mass=5.0, friction=2.0, actuator=1.0, mass_band=0.10, friction_band=0.10, actuator_band=0.05
    ),
    EvalScenario(
        "OOD Ultra Heavy", mass=35.0, friction=1.0, actuator=1.0, mass_band=0.05, friction_band=0.05, actuator_band=0.02
    ),
    EvalScenario(
        "OOD Weak Motor", mass=10.0, friction=1.0, actuator=0.45, mass_band=0.05, friction_band=0.05, actuator_band=0.02
    ),
    EvalScenario(
        "OOD Heavy Weak", mass=30.0, friction=1.0, actuator=0.55, mass_band=0.05, friction_band=0.05, actuator_band=0.02
    ),
]

# Eval target jitter for protocol v2 (metres, uniform around config.target_pos).
EVAL_TARGET_JITTER_V2 = 0.5
...
def build_domain_randomization_config(config: Stage2Config, for_eval: bool = False) -> dict:
    return {
        ...
        "actuator_strength_range": config.actuator_strength_range,
        ...
        "disturbance_actuator_scale_range": config.disturbance_actuator_scale_range,
...
def train_one_seed(seed: int, config: Stage2Config) -> tuple[Path, pd.DataFrame]:
    ...
    for update in pbar:
        ...
        if np.isnan(training_rows[-1]["recent_episode_return_mean"]):
            reward_text = "nan"
        tmin = trackers.get("current_target_min", config.target_min)
        tmax = trackers.get("current_target_max", config.target_max)
        pbar.set_postfix(
            reward=reward_text,
            episodes=len(trackers["completed_returns"]),
            target_range=f"{tmin:.1f}-{tmax:.1f}",
        )
...
def _eval_nominals(env) -> dict:
    """Capture XML-nominal physics once per env instance (before any scenario
    writes). Used to normalise context exactly like DomainRandomizationWrapper
    does during training (mass_scale = mass / XML nominal, etc.)."""
    base = env.unwrapped
    if not hasattr(base, "_eval_nominals_cache"):
        base._eval_nominals_cache = {
            "mass": float(base.model.body_mass[1]),
            "friction": float(base.model.geom_friction[0, 0]),
            "gainprm": base.model.actuator_gainprm.copy(),
            "jnt_actfrcrange": base.model.jnt_actfrcrange.copy(),
        }
    return base._eval_nominals_cache


def apply_eval_physics(env, mass: float, friction: float, actuator: float) -> None:
    """Write scenario physics into the model and push the TRUE context to the
    env observation. The original (v1) static eval omitted the context push,
    so the context agent always observed (1.0, 1.0) — far outside its training
    context distribution (mass_scale trained range ~[11.6, 46.5])."""
    nom = _eval_nominals(env)
    base = env.unwrapped
    base.model.body_mass[1] = float(mass)
    base.model.geom_friction[0, 0] = float(friction)
    base.model.actuator_gainprm[:, 0] = nom["gainprm"][:, 0] * float(actuator)
    base.model.jnt_actfrcrange[:] = nom["jnt_actfrcrange"] * float(actuator)
    base.set_disturbance_context(
        mass_scale=float(mass) / max(nom["mass"], 1e-9),
        friction_scale=float(friction) / max(nom["friction"], 1e-9),
        actuator_scale=float(actuator),
    )


def prepare_eval_episode(env, scenario: EvalScenario, seed: int, config: "Stage2Config | None" = None):
    protocol = getattr(config, "eval_protocol", "v1") if config is not None else "v1"

    if protocol == "v2":
        rng = np.random.default_rng(seed)
        mass = scenario.mass * (1.0 + rng.uniform(-scenario.mass_band, scenario.mass_band))
        friction = scenario.friction * (1.0 + rng.uniform(-scenario.friction_band, scenario.friction_band))
        actuator = scenario.actuator * (1.0 + rng.uniform(-scenario.actuator_band, scenario.actuator_band))
        target = float(config.target_pos + rng.uniform(-EVAL_TARGET_JITTER_V2, EVAL_TARGET_JITTER_V2))

        apply_eval_physics(env, mass, friction, actuator)
        env.unwrapped.set_target_pos(target)
        obs, info = env.reset(seed=seed)
        # reset_model zeroes the context; re-push the true values.
        apply_eval_physics(env, mass, friction, actuator)
        env.unwrapped._last_eval_physics = {
            "mass": float(mass),
    ...
    # v1 (legacy): exact scenario physics, no context push — kept for
    # reproducing the original evaluation of old checkpoints.
...
def run_eval_episode(eval_env, agent, scenario: EvalScenario, seed: int, config: Stage2Config) -> dict:
    obs, _ = prepare_eval_episode(eval_env, scenario, seed, config)
    positions = []
    rewards = []
    # env.dt = model.opt.timestep * frame_skip — positions are recorded once per
    # control step, so settling time must use the control-step period (0.02 s).
    ...
    sampled = getattr(eval_env.unwrapped, "_last_eval_physics", {})
    return {
        "seed": seed,
        "scenario": scenario.name,
        "episode": int(seed % 1_000_000),
        "mass": sampled.get("mass", scenario.mass),
        "friction": sampled.get("friction", scenario.friction),
        "actuator": sampled.get("actuator", scenario.actuator),
        "target": sampled.get("target", float(eval_env.unwrapped.target_pos)),
...
def evaluate_model(seed: int, model_path: Path, config: Stage2Config) -> pd.DataFrame:
    ...
    scenario_list = SCENARIOS_V2 if config.eval_protocol == "v2" else SCENARIOS
    records = []
...
def plot_eval_trajectories(seed: int, model_path: Path, config: "Stage2Config", output_path: Path) -> None:
    ...
    plot_scenarios = SCENARIOS_V2 if config.eval_protocol == "v2" else SCENARIOS
    fig, axes = plt.subplots(len(plot_scenarios), 3, figsize=(15, 4 * len(plot_scenarios)))
    ...
    for i, scenario in enumerate(plot_scenarios):
        obs, _ = prepare_eval_episode(env, scenario, seed=70000, config=config)
        hidden_state = agent.get_initial_state(1, device=device) if config.recurrent_policy else None
        done_tensor = torch.zeros(1, device=device) if config.recurrent_policy else None
        ...
        row_label = f"{scenario.name}\n(m={scenario.mass} kg, fr={scenario.friction}, act={scenario.actuator})"
        ep_target = getattr(env.unwrapped, "_last_eval_physics", {}).get("target", target)

        ax = axes[i, 0]
        ax.plot(t, pos, color=color, lw=1.5)
        ax.axhline(ep_target, color="black", lw=1, linestyle=":", label=f"Target {ep_target:.2f} m")
...
def protocol_preset_defaults(preset_name: str) -> dict[str, Any]:
    ...
    if preset_name == "thesis_v6_hipmdp":
        # Hidden-parameter MDP protocol: thesis_v4_cliff reward/curriculum
        # structure + the actuator-strength axis as a second, dynamics-relevant
        # hidden parameter (terminal speed scales with actuator strength, so
        # this axis is discriminative where friction was not). Eval protocol v2:
        # per-episode physics bands, target jitter, true context push.
        return {
            "randomize_target": True,
            "target_pos": 5.0,
            "target_min": 3.0,
            ...
            "disturbance_mass_scale_end_range": "0.9,1.3",
            "disturbance_friction_scale_end_range": "0.5,1.4",
            "disturbance_actuator_scale_range": "0.85,1.15",
            "position_patch_enabled": True,
            "patch_x_range": "1.5,2.4",
            "patch_friction_scale": 0.35,
            "safety_speed_governor_enabled": False,
            ...
            "near_target_init_prob": 0.20,
            "near_target_init_range_m": 0.04,
            # The new axis + honest eval:
            "actuator_strength_range": "0.6,1.4",
            "eval_protocol": "v2",
        }
...
def build_config(args: argparse.Namespace) -> Stage2Config:
    ...
    if obs_keep_dims == 0:
        # Full observation: 9 dims since the actuator-strength context was added.
        # Old 8-dim checkpoints are handled by the inference branch above.
        obs_keep_dims = 9
    if obs_keep_dims <= 0 or obs_keep_dims > 9:
        raise ValueError(f"obs-keep-dims must be in [1, 9], got {obs_keep_dims}")
    ...
    return Stage2Config(
        ...
        actuator_strength_range=parse_range_pair(args.actuator_strength_range, float),
        disturbance_actuator_scale_range=parse_range_pair(args.disturbance_actuator_scale_range, float),
...
def save_json_config(config: Stage2Config) -> None:
    ...
    scenario_list = SCENARIOS_V2 if config.eval_protocol == "v2" else SCENARIOS
    serializable["scenarios"] = [asdict(s) for s in scenario_list]
...
def main() -> None:
    ...
    parser.add_argument(
        "--actuator-strength-range",
        type=str,
        default="1.0,1.0",
        help="Per-episode actuator strength randomization range (1.0,1.0 = off)",
    )
    parser.add_argument(
        "--disturbance-actuator-scale-range",
        type=str,
        default="1.0,1.0",
        help="Mid-episode actuator strength shift range (1.0,1.0 = off)",
    )
    parser.add_argument(
        "--eval-protocol",
        type=str,
        choices=["v1", "v2"],
        default="v1",
        help="v1 = legacy deterministic scenarios (no context push); "
        "v2 = per-episode physics bands + target jitter + true context push",
    )
