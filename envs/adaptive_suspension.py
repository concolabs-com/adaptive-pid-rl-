...
class AdaptiveSuspensionEnv(MujocoEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Obs: [pos, vel, err, a_kp, a_ki, a_kd, mass_scale, friction_scale, actuator_scale]
        ...
        self._disturbance_context = np.array([1.0, 1.0, 1.0], dtype=np.float64)

    def set_disturbance_context(self, mass_scale: float, friction_scale: float, actuator_scale: float = 1.0) -> None:
        self._disturbance_context[0] = float(mass_scale)
        self._disturbance_context[1] = float(friction_scale)
        self._disturbance_context[2] = float(actuator_scale)
